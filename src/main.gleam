...
fn start_presence_sensor_actor() -> Result(
  Subject(presence_sensor.PresenceSensorMessage),
  String,
) {
  presence_sensor.actor()
...
fn start_wisp(
  ps_actor: Subject(presence_sensor.PresenceSensorMessage),
  config: Configuration,
) -> Result(Nil, String) {
  wisp_mist.handler(router.handle_request(_, ps_actor), wisp.random_string(64))
  |> mist.new
  |> mist.bind("0.0.0.0")
